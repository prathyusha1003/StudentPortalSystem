...
public class FacultyController 
{
	...
	public String sendResetOtp(@RequestParam String email, ModelMap map) throws UnsupportedEncodingException, MessagingException
	{
    ...
    public String fetchStudentDetails(HttpSession session, ModelMap map)
    {
		...
		{
			return facultyService.fetchStudentDetails(map);
		}
    }
    ...
    public String fetchStudentNames(HttpSession session, ModelMap map)
    {
		...
		{
			return facultyService.fetchStudentNames(map);
		}
    }
	
	@PostMapping("/attendence")
    public String addAttendence(HttpSession session, ModelMap map,@RequestParam int id,@RequestParam String attend)
    {
		Faculty faculty = (Faculty) session.getAttribute("faculty");
		if(faculty == null)
		{
		...
		{
			return facultyService.addAttendence(id,attend,map);
		}
    }
}
